...
class DDPM():
    ...
    def sample_forward(self, x, t, eps=None):
        alpha_bar = self.alpha_bars[t].reshape(-1, 1, 1, 1)  # 每次取一个batch的数据
        if eps is None:
            eps = torch.randn_like(x)
        res = eps * torch.sqrt(1 - alpha_bar) + torch.sqrt(alpha_bar) * x  # 正向过程计算x_t，加随机噪声
        return res

    def sample_backward(self, img_shape, net, device, simple_var=True):
        x = torch.randn(img_shape).to(device)  # 随机采样噪声（相当于加噪后图像）
        net = net.to(device)
        for t in range(self.n_steps - 1, -1, -1):
            x = self.sample_backward_step(x, t, net, simple_var)  # 反向过程，逐步去噪
        return x
    ...
    def sample_backward_step(self, x_t, t, net, simple_var=True):
        ...
        eps = net(x_t, t_tensor)  # 预测的附加噪声

        if t == 0:
            noise = 0
        else:
            if simple_var:  # 方差项的不同取法
                var = self.betas[t]
            else:
