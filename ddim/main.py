...
def sample_imgs(ddpm,
                ...
                **kwargs):
    if img_shape[1] >= 256:
        max_batch_size = 16  # 根据图像尺寸调整batch_size
    ...
    with torch.no_grad():
        while n_sample > 0:
            if n_sample >= max_batch_size:
                ...
                batch_size = n_sample  # 取较小的batch_size
            n_sample -= batch_size
            ...
            imgs = ddpm.sample_backward(shape,
                                        ...
                                        **kwargs).detach().cpu()
            imgs = (imgs + 1) / 2 * 255
            imgs = imgs.clamp(0, 255).to(torch.uint8)  # 一次性计算出batch_size张图像
            ...
            for i, img in enumerate(img_list):
                ...
                cv2.imwrite(f'{output_dir}/{i + index}.jpg', img)  # 逐个图像保存

            # First iteration
            if index == 0:
                imgs = einops.rearrange(imgs,
                                        '(b1 b2) c h w -> (b1 h) (b2 w) c',
                                        b1=int(batch_size ** 0.5))  # 合成一幅大图
                imgs = imgs.numpy()
                if to_bgr:
...
if __name__ == '__main__':
    ...
    sample_imgs(ddim,
                ...
                device=device,
                to_bgr=to_bgr,
                ddim_step=1000,  # 不加速采样
                simple_var=False,  # 不使用原始DDPM默认方差
                eta=0)  # 使用广义DDPM eta0
    # 后续三种为DDIM加速采样
    sample_imgs(ddim,
                net,
                ...
                device=device,
                simple_var=True,
                to_bgr=to_bgr)  # NOTE:how to use DDPM var?
    sample_imgs(ddim,
                net,
