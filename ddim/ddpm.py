...
class DDPM():
    ...
    def sample_backward(self, img_or_shape, net, device, simple_var=True):
        if isinstance(img_or_shape, torch.Tensor):
            x = img_or_shape  # 实际的图像
        else:
            x = torch.randn(img_or_shape).to(device)  # shape
        net = net.to(device)
    ...
    def sample_backward_step(self, x_t, t, net, simple_var=True):
        ...
        if t == 0:
            ...
            if simple_var:
                var = self.betas[t]  # DDPM实际效果最好方差
            else:
                var = (1 - self.alpha_bars[t - 1]) / (1 - self.alpha_bars[t]) * self.betas[t]  # 理论方差
            noise = torch.randn_like(x_t)
            noise *= torch.sqrt(var)

        mean = (x_t - (1 - self.alphas[t]) / torch.sqrt(1 - self.alpha_bars[t]) * eps) / torch.sqrt(self.alphas[t])
        x_t = mean + noise  # 采样公式 p(Xt-1,Xt;X0)
